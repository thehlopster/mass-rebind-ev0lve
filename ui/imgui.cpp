...
static void ImGui::UpdateMouseInputs()
{
	...
	for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
	{
		...
		g.IO.MouseUnclicked[i] = !g.IO.MouseDown[i] && g.IO.MouseUpDuration[i] < 0.0f;
		g.IO.MouseReleased[i] = !g.IO.MouseDown[i] && g.IO.MouseDownDuration[i] >= 0.0f;
		g.IO.MouseDownDurationPrev[i] = g.IO.MouseDownDuration[i];
		g.IO.MouseDownDuration[i] = g.IO.MouseDown[i] ? (g.IO.MouseDownDuration[i] < 0.0f ? 0.0f : g.IO.MouseDownDuration[i] + g.IO.DeltaTime) : -1.0f;
		g.IO.MouseUpDuration[i] = !g.IO.MouseDown[i] ? (g.IO.MouseUpDuration[i] < 0.0f ? 0.0f : g.IO.MouseUpDuration[i] + g.IO.DeltaTime) : -1.0f;
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	flags |= ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
	...
	if (flags & ImGuiWindowFlags_Listbox)
		size.x = 165.f;
	...
	if (flags & ImGuiWindowFlags_Listbox)
		parent_window->DC.CursorPos.x += 1;
...
void ImGui::EndChild()
{
	...
	const bool is_correct_group = flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip) && !(flags & ImGuiWindowFlags_Listbox);
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
	...
	const bool is_correct_group = flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip);
	...
	// Shadow (only for popups and tooltips)
	if (flags & ImGuiWindowFlags_Popup || flags & ImGuiWindowFlags_Tooltip || flags & ImGuiWindowFlags_Listbox) {
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
	...
	const bool has_collapse_button = false;
	...
	ImRect layout_r(title_bar_rect.Min.x + pad_l + 6.f, title_bar_rect.Min.y, title_bar_rect.Max.x - pad_r, title_bar_rect.Max.y);
	...
	if (first_begin_of_the_frame)
	{
		...
		if (flags & ImGuiWindowFlags_ChildWindow)
		{
			...
			if (!(flags & ImGuiWindowFlags_Popup) && !window_pos_set_by_api && !window_is_child_tooltip)
				window->Pos = parent_window->DC.CursorPos + ImVec2(24.5f, 0);
		}
		...
		window->InnerRect.Min.y = window->Pos.y;
		window->InnerRect.Max.x = window->Pos.x + window->Size.x - window->ScrollbarSizes.x;
		window->InnerRect.Max.y = window->Pos.y + window->Size.y - window->ScrollbarSizes.y;

		if (is_correct_group)
			window->InnerRect.Min.y += window->TitleBarHeight();
...
void ImGui::OpenPopupEx(ImGuiID id)
{
	...
	{
		// Gently handle the user mistakenly calling OpenPopup() every frame. It is a programming mistake! However, if we were to run the regular code path, the ImGui
