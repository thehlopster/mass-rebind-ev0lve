...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
	...
	if (CalcTextSize(text, 0, true).x == 0.f)
		return;
	...
	ImVec2 text_pos(window->DC.CursorPos.x + 24.5f, window->DC.CursorPos.y + 2.f);
	if (window->Flags & ImGuiWindowFlags_Tooltip)
		text_pos = ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y);
	...
	else
	{
		...
		const ImVec2 text_size = CalcTextSize(text_begin, text_end, true, wrap_width);
		...
		RenderText(bb.Min, text_begin, text_end);
	}
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	ImVec2 pos = window->DC.CursorPos + ImVec2(24.5f, 0);
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
		pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
	ImVec2 size = CalcItemSize(size_arg, 165.f, label_size.y + style.FramePadding.y * 2.0f);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
	...
	ScrollbarEx(bb, id, axis, &window->Scroll[axis], inner_rect.Max[axis] - inner_rect.Min[axis], window->ContentSize[axis] + window->WindowPadding[axis] * 2.0f, 0);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	const float w = 165.f;
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(24.5f, 0), window->DC.CursorPos + ImVec2(24.5f, 0) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton))
	{
		...
		if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
			RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 4.f), text_col, popup_open ? ImGuiDir_Up : ImGuiDir_Down, 0.5f);
	}

	if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
		RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(2.f, 0), ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
	...
	if (ImGuiWindow * popup_window = FindWindowByName(name))
		if (popup_window->WasActive)
		{
			...
			SetNextWindowPos(pos + ImVec2(0, 4.f));
		}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	RenderTextClipped(frame_bb.Min + ImVec2(0, 4.f), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

	if (g.IO.KeyCtrl && hovered && data_type == ImGuiDataType_Float)
		* (float*)v = floorf(*(float*)v);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = CalcItemSize(size_arg, 165.f, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(24.5f, 0), window->DC.CursorPos + ImVec2(24.5f, 0) + size);
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
	...
	if (!buf[0])
		window->DrawList->AddText(frame_bb.Min + ImVec2(4.f, 2.f), ImColor(140, 140, 140), label);
	...
	if (render_cursor || render_selection)
	{
		...
		if (render_selection)
		{
			...
			ImU32 bg_color = ImColor(239, 9, 95, 120); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	flags |= ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_AlphaBar;
	...
	ImVec2 pos = window->DC.CursorPos;
	pos.x = window->Pos.x + window->Size.x - square_sz - style.FramePadding.x * 2.f - style.ItemInnerSpacing.x - window->ScrollbarSizes[1];
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		if (col_v4.x != o_col.x || col_v4.y != o_col.y || col_v4.z != o_col.z || col_v4.w != o_col.w)
		{
			col[0] = col_v4.x;
			col[1] = col_v4.y;
			col[2] = col_v4.z;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	else if (flags & ImGuiColorEditFlags_PickerHueBar)
	{
		...
		float bar0_line_y = ImClamp((float)(int)(picker_pos.y + H * sv_picker_size + 0.5f), picker_pos.y + 2, picker_pos.y + sv_picker_size - 1);
		RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 0.0f);

		draw_list->AddRect(ImVec2(bar0_pos_x + 1, bar0_line_y - 1), ImVec2(bar0_pos_x + bars_width, bar0_line_y + 1), ImColor(0, 0, 0));
		//RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f);
	}

	// Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
	draw_list->AddRect(sv_cursor_pos - ImVec2(1, 1), sv_cursor_pos + ImVec2(2, 2), ImColor(0, 0, 0));
	...
	if (alpha_bar)
	{
		...
		float bar1_line_y = ImClamp((float)(int)(picker_pos.y + (1.0f - alpha) * sv_picker_size + 0.5f), bar1_bb.Min.y + 2, bar1_bb.Max.y - 1);
		RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 0.0f);
		draw_list->AddRect(ImVec2(bar1_pos_x + 1, bar1_line_y - 1), ImVec2(bar1_pos_x + bars_width, bar1_line_y + 1), ImColor(0, 0, 0));
	}
...
bool ImGui::ColorButton(const char* desc_id, ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	bool popup_open = IsPopupOpen(id);
	if (!pressed && hovered && g.IO.MouseClicked[1]) {
		OpenPopupEx(id);
		popup_open = true;
	}

	if (popup_open) {
		SetNextWindowSizeConstraints(ImVec2(60.f, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(-1)));
		...
		// Horizontally align ourselves with the framed text
		ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings;
		PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(GetStyle().FramePadding.x, GetStyle().WindowPadding.y));
		bool ret = Begin(name, NULL, window_flags);
		PopStyleVar();
		if (!ret)
		{
		...
		{
			char buffer[9];
			ImFormatString(buffer, 9, "%02X%02X%02X%02X", (int)(col_rgb.x * 255.f), (int)(col_rgb.y * 255.f), (int)(col_rgb.z * 255.f), (int)(col_rgb.w * 255.f));
		...
		if (Selectable("Paste", false)) {
			...
			unsigned int val = IM_COL32(0, 0, 0, 255);
			auto retn = sscanf(cb, "%X", &val);

			if (retn)
			{
				auto a = val & 0xFF; val >>= 8;
				auto b = val & 0xFF; val >>= 8;
				auto g = val & 0xFF; val >>= 8;
				auto r = val & 0xFF;
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
	...
	ImRect frame_bb(window->DC.CursorPos + ImVec2(24.5f, 0), window->DC.CursorPos + ImVec2(24.5f, 0) + frame_size);
	...
	BeginChildFrame(id, frame_bb.GetSize(), ImGuiWindowFlags_Listbox | ImGuiWindowFlags_NoTitleBar);
	return true;
}
...
bool ImGui::TabButton(const char* text, int id, int max, int selected) {
	auto window = GetCurrentWindow();
	if (window->SkipItems)
		return false;

	auto width = window->Size.x / max;
	auto bb = ImRect(
		ImVec2(window->Pos.x + width * id, window->Pos.y + window->TitleBarHeight()),
		ImVec2(window->Pos.x + width * (id + 1), window->Pos.y + window->TitleBarHeight() + 30));

	ItemSize(bb);
	if (!ItemAdd(bb, GetID(text)))
		return false;

	bool hovered = false, held = false;
	auto clicked = ButtonBehavior(bb, GetID(text), &hovered, &held);


	auto text_size = CalcTextSize(text);
	auto size = bb.Max - bb.Min;

	auto text_pos = bb.Min + ImVec2(size.x * 0.5f - text_size.x * 0.5f, size.y * 0.5f - text_size.y * 0.5f);

	if (selected != id) {
		if (hovered)
			window->DrawList->AddText(text_pos, ImColor(170, 170, 170), text);
		else
			window->DrawList->AddText(text_pos, ImColor(51, 51, 51), text);
	}
	else
		window->DrawList->AddText(text_pos, ImColor(255, 255, 255), text);

	return clicked;
}

bool ImGui::SingleSelect(const char* text, int* sel, std::vector<const char*> items) {
	*sel = std::clamp(*sel, 0, (int)items.size() - 1);

	auto old_val = *sel;
	if (BeginCombo(text, items.at(*sel))) {
		for (int i = 0; i < items.size(); i++) {
			if (Selectable(items.at(i), *sel == i))
				* sel = i;
		}
...
bool ImGui::SingleSelectStr(const char* text, int* sel, std::vector<std::string> items) {
	auto old_val = *sel;
	if (BeginCombo(text, items.at(*sel).c_str())) {
		for (int i = 0; i < items.size(); i++) {
			if (Selectable(items.at(i).c_str(), *sel == i))
				*sel = i;
		}
...
bool ImGui::MultiSelect(const char* text, bool* sel, std::vector<const char*> items) {
	auto old_data = *sel;
	...
	if (BeginCombo(text, selected > 0 ? (std::to_string(selected) + " selected").c_str() : "None")) {
		for (int i = 0; i < items.size(); i++) {
			if (Selectable(items.at(i), sel[i], ImGuiSelectableFlags_DontClosePopups))
				sel[i] = !sel[i];
		}
...
bool ImGui::Hotkey(const char* text, int* key, int* behaviour) {
	static std::unordered_map<int, const char*> keys = {
	...
	static std::unordered_map<int, const char*> behaviours = {
		{ 0, "always on" }, { 1, "hold" }, { 2, "toggle" }, { 3, "inverse" }
	};
	...
	auto pos = window->DC.CursorPos + ImVec2(24.5f, 0);
	auto frame = ImRect(pos, pos + ImVec2(165.f, GetFrameHeight()));
	auto bb = ImRect(frame.Min + ImVec2(0, 4.f), frame.Max);

	ItemSize(frame);
	if (!ItemAdd(frame, GetID(text)))
		return false;

	bool held, hovered;
	bool pressed = ButtonBehavior(bb, GetID(text), &hovered, &held);

	auto old_key = *key;
	auto old_behaviour = behaviour ? *behaviour : 0;

	char new_text[2048];
	memset(new_text, 0, 2048);

	ImFormatString(new_text, 2048, "%s1337", text);

	auto id = GetID(new_text);
	auto is_active = GetActiveID() == id;
...
DRAW:
	...
	if (behaviour)
		ImFormatString(render_text, IM_ARRAYSIZE(render_text), "%s (%s)", keys[*key] ? keys[*key] : "none", behaviours[*behaviour]);
	else
		ImFormatString(render_text, IM_ARRAYSIZE(render_text), "%s", keys[*key] ? keys[*key] : "none");

	RenderTextClipped(bb.Min + ImVec2(4.f, 0), bb.Max, render_text, 0, 0);

	return old_key != *key || (behaviour ? *behaviour != old_behaviour : false);
}
