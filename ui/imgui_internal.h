...
static inline ImVec2  ImCeil(ImVec2 x)                                            { return ImVec2(ceilf(x.x), ceilf(x.y)); }
#endif
...
struct IMGUI_API ImPool
{
    ...
    ImPool()    { FreeIdx = 0; }
    ~ImPool()   { Clear(); }
    T*          GetByKey(ImGuiID key)               { int idx = Map.get_int(key, -1); return (idx != -1) ? &Data[idx] : NULL; }
...
public:
    ...
    float       TitleBarHeight() const      { ImGuiContext& g = *GImGui; return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.0f : CalcFontSize() + g.Style.FramePadding.y * 2.0f + 6.f; }
