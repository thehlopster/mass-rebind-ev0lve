...
#define IMGUI_CHECKVERSION()        ui::DebugCheckVersionAndDataLayout(IMGUI_VERSION, sizeof(ImGuiIO), sizeof(ImGuiStyle), sizeof(ImVec2), sizeof(ImVec4), sizeof(ImDrawVert), sizeof(ImDrawIdx))
...
namespace ImGui
{
    ...
    IMGUI_API bool          TabButton(const char* text, int id, int max, int selected);
    IMGUI_API bool          SingleSelect(const char* text, int* sel, std::vector<const char*> items);
    IMGUI_API bool          SingleSelectStr(const char* text, int* sel, std::vector<std::string> items);
    IMGUI_API bool          MultiSelect(const char* text, bool* sel, std::vector<const char*> items);
    IMGUI_API bool          Hotkey(const char* text, int* key, int* behaviour = nullptr);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_Listbox                = 1 << 29,

	ImGuiWindowFlags_MessageBox				= 1 << 30
...
struct ImGuiIO
{
    ...
    bool        MouseUnclicked[5];                // Mouse button went from !Down to Down
    ...
    float       MouseUpDuration[5];           // Duration the mouse button has been down (0.0f == just clicked)
...
struct ImFontAtlas
{
    ...
    IMGUI_API const ImWchar*    GetGlyphRangesAll();             // Default + Vietnamese characters
