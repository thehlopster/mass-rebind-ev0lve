...
void c_ui_slider_float::draw(c_ui_window *wnd) {
    ...
    if (val)
        d->AddRectFilled(slider_area.Min + ImVec2(0.f, 5.f),
                         slider_area.Min + ImVec2(((float)(*val) - (float)min) / ((float)max - (float)min) * slider_w, slider_h - 5.f), ui->style.e_accent);
    ...
    if (val) {
        ...
        d->AddTextShadow(slider_area.Min + ImVec2(slider_area.GetWidth() / 2 - text_size.x / 2, 3.f),
