...
void c_ui_dropdown::setup(std::string n) {
    setup(n, nullptr);
}

void c_ui_dropdown::draw(c_ui_window *wnd) {
    ...
    if (ui->active_id == id && ui->is_unclicked() && !ui->is_hovered(control_rect)) {
        clear = true;
    ...
    if (!val || dd_elements.empty())
        d->AddTextShadow(control_rect.Min + ImVec2(5.f, 3.f), ui->style.e_text_disabled, "-");
    else
        d->AddTextShadow(control_rect.Min + ImVec2(5.f, 3.f), ui->style.e_text, dd_elements.at(*val).c_str());
    d->PopClipRect();
...
void c_ui_dropdown::setup(std::string n, int *v) {
    name = std::move(n);
